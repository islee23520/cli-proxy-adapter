...
function gptFastTierBaseId(mid) {
  return mid.startsWith("gpt-") && mid.endsWith("-fast")
    ? mid.slice(0, -"-fast".length)
    : null;
}
...
async function fetchModels(baseUrl, apiKey, timeoutMs) {
  ...
  try {
    ...
    const ids = (json.data || [])
      .map((m) => m.id)
      .filter(Boolean)
      // Fable is offered by Cursor directly; do not surface it through CLIProxy.
      .filter((id) => !String(id).toLowerCase().includes("fable"))
      .sort();
...
function backendSearchSupport(mid, catalog, searchModelIds) {
  const slug = mid.includes("/") ? mid.slice(mid.lastIndexOf("/") + 1) : mid;
  if (searchModelIds.has(mid) || searchModelIds.has(slug)) return true;
  const cat = catalog.byId.get(mid) || catalog.byId.get(slug);
  if (cat && cat.backendSearch === true) return true;
  return mid === "grok-4.20-multi-agent-0309";
}
...
function buildManagedToml({
  ...
  const lines = [];
  const searchModelIds = new Set(
    [webSearch, xSearch].filter((id) => typeof id === "string" && id.trim()),
  );
  ...
  for (const mid of ids) {
    ...
    if (backendSearchSupport(mid, catalog, searchModelIds)) {
...
async function main() {
  ...
  const built = buildManagedToml({
    ...
    xSearch: cfg.xSearch || cfg.webSearch || "grok-4.20-multi-agent-0309",
