...
test("verify-hosts composes active metadata from a faithful isolated model fixture on all four hosts", async () => {
	...
	const piFamilyOutput = "cpa gpt-5.5 272K 128K\ncpa gpt-5.6-terra 1.1M 128K\ncpa grok-build-0.1 256K no";
	...
	for (const toml of [managedToml, activeToml]) {
		...
		expect(section(toml, 'model."gpt-5.6-sol"')).toContain("context_window = 1050000");
		expect(section(toml, 'model."gpt-5.6-terra"')).toContain("context_window = 1050000");
...
test("verify-hosts skips absent original CLIs on a fork-only machine without failing", async () => {
	...
	const bin = join(sandbox, "bin");
	await mkdir(bin, { recursive: true });
	await writeCommand(join(bin, "senpi"), "cpa gpt-5.5 272K 128K\ncpa gpt-5.6-terra 1.1M 128K\ncpa grok-build-0.1 256K no");
