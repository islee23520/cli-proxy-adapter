...
const PI_MODEL_EXPECTATIONS = [
	...
	{ query: "gpt-5.6-terra", needles: ["cpa", "gpt-5.6-terra", "1.1M", "128K"], detail: "1.05M context / 128K output" },
...
function verifyGrokComposition(): void {
	...
	try {
		...
		for (const [label, toml] of [["managed TOML", managedToml], ["active GROK_CONFIG", activeToml]] as const) {
			requireSectionValue(toml, 'model."gpt-5.6-terra"', "context_window", "1050000");
			record("PASS", `grok-family ${label}`, `${label} contains gpt-5.6-terra at 1.05M`);
		}
...
const families: readonly Family[] = [
	...
	},
	{
		...
		shared: {
			...
			detail: "non-dry-run composition proves Terra at 1.05M in managed and active TOML",
		},
	},
];
